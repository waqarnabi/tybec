...
int main(int argc, char** argv)
{
    ...
    int h_axi01_ptr0_input[MAX_LENGTH];                    // host memory for input vector
    int h_axi01_ptr0_output[MAX_LENGTH];                   // host memory for output vector
   ...
   // Get Accelerator compute device
    ...
    //
     kernel = clCreateKernel(program, "ktop", &err);
    ...
    d_axi01_ptr0 = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX,  sizeof(int) * number_of_words, &d_bank_ext[0], NULL);


    if (!(d_axi00_ptr0&&d_axi01_ptr0)) {
    ...
    err = clEnqueueWriteBuffer(commands, d_axi01_ptr0, CL_TRUE, 0, sizeof(int) * number_of_words, h_axi01_ptr0_input, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to source array h_axi01_ptr0_input!\n");
        printf("Test failed\n");
        return EXIT_FAILURE;
    }
    ...
    err |= clSetKernelArg(kernel, 3, sizeof(cl_mem), &d_axi00_ptr0); // Not used in example RTL logic.
    ...
    err |= clEnqueueReadBuffer( commands, d_axi01_ptr0, CL_TRUE, 0, sizeof(int) * number_of_words, h_axi01_ptr0_output, 0, NULL, &readevent );
    ...
    for (uint i = 0; i < number_of_words; i++) {
        if ((h_axi01_ptr0_input[i] + 1) != h_axi01_ptr0_output[i]) {
            printf("ERROR in ktop - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_axi01_ptr0_input[i], h_axi01_ptr0_input[i], h_axi01_ptr0_output[i], h_axi01_ptr0_output[i]);
            check_status = 1;
        }
