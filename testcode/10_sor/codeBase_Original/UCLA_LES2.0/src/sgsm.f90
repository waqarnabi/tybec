...
contains 
  ...
  subroutine diffuse
    ...
    use grid, only : a_up, a_uc, a_ut, a_vp, a_vc, a_vt, a_wp, a_wc, a_wt    &
         , a_rv, a_rp, a_tp, a_tt, a_sp, a_st, a_qt, a_qp, a_pexnr, a_theta  &
    ...
    if (level  > 1) then 
       call bruvais(nzp,nxp,nyp,level,a_theta,a_tp,a_rp,a_scr2,a_scr3,dzm   &
            ,th00)
    else
    ...
    do n=1,nscl
       ...
       if (sflg) then
          ...
          if (associated(a_sp,a_tp))                                          &
             call sgsflxs(nzp,nxp,nyp,level,a_scr3,a_rv,a_theta,a_scr1,'tl')
          if (associated(a_sp,a_rp))                                          &
             call sgsflxs(nzp,nxp,nyp,level,a_scr3,a_rv,a_theta,a_scr1,'rt')
       endif
